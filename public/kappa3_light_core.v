...
module kappa3_light_core(input 	       clock,
   ...
   assign dbg_pc_out = pc;
    ...
    function [31:0] pc_select(input pc_sel,input [31:0] pc,input [31:0] creg);
 		case(pc_sel)
 		0:pc_select = pc + 4;
 		1:pc_select = creg;
 		endcase
    ...
    assign pc_in = pc_select(pc_sel,pc,creg);
   // IR
   ...
   memory mem_inst(.clock(clock),
	...
	assign ldconv_offset1 = mem_select(mem_sel,pc,creg);
	assign ldconv_offset = ldconv_offset1[1:0];
	
	 function[31:0] mem_select(input mem_sel,input [31:0] pc,input [31:0] creg);
	 	case(mem_sel)
	 		0:mem_select = pc;
	 		1:mem_select = creg;
	 	endcase
	 ...
	 assign mem_addr = mem_select(mem_sel,pc,creg);
   // reg-file
   ...
   alu alu_inst(.in1(in1),
	   ...
	   .out(alu_out));
	// ALU assign
	function[31:0] a_select(input a_sel,input [31:0] areg,input [31:0] pc);
		case(a_sel)
		0: a_select = areg;
		1: a_select = pc;
		endcase
    endfunction
    function[31:0] b_select(input b_sel,input [31:0] breg,input [31:0] imm);
   	case(b_sel)
		0: b_select = breg;
		1: b_select = imm;
		endcase
    endfunction
   assign in1 = a_select(a_sel,areg,pc);
   assign in2 = b_select(b_sel,breg,imm);
   
	
	function[31:0] rd_select(input [1:0] rd_sel,input [31:0] ldconv_out,input [31:0] pc,input [31:0] creg);
	 	case(rd_sel)
	 	2'b00: rd_select = ldconv_out;
	 	2'b01: rd_select = pc + 4;
	 	2'b10: rd_select = creg;
		default:;
	 	endcase
	 endfunction

	 assign rd_in = rd_select(rd_sel,ldconv_out,pc,creg);
	 //assign rd_in = ldconv_out;
